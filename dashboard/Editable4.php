...
if(isset($_POST["submit"])){
        foreach ($_POST["id"] AS $id){
                            ...
                            $field3 = mysqli_real_escape_string($conn,$_POST["Day"][$id]);
                            $update="UPDATE `programs` 
                            SET `Title` = '$field1', `Paragraph` = '$field2', `Day` = '$field3'
                            WHERE `id` = $id LIMIT 1 ";
...
$sql="SELECT * FROM programs ORDER BY id DESC";
            ...
            if( mysqli_num_rows($res) > 0){
				echo '<form method="post" action="Editable4.php" enctype="multipart/form-data">';
				echo '
                ...
                while($row=mysqli_fetch_assoc($res)){
                    echo'
                            ...
                            <td><input class="form-control" type="text" name="Day['.$row["id"].']" value="'.$row["Day"].'"></td>
